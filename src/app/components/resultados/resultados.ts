...
import { RegistroResultados } from '../../models/user.models';
...
export class Resultados implements OnInit {
  ...
  historialCompleto = signal<RegistroResultados[]>([]);
  ...
  resultadosMayorMenor = computed(() => {
    return this.historialCompleto()
      .filter(r => r.juego === 'MAYOR-O-MENOR')
  ...
  resultadosClickCircle = computed(() => {
    return this.historialCompleto()
      .filter(r => r.juego === 'CLICK-THE-CIRCLE')
  ...
  async cargarEstadisticas() {
    ...
    try {
      ...
      const datos = await this.registrosService.obtenerResultados();
